# ROCm 7.2 Windows Setup Plan for ACE-Step 1.5

## Overview
Complete step-by-step plan for setting up ACE-Step 1.5 with ROCm 7.2 on Windows for AMD Radeon 780M (gfx1102) with 16GB UMA.

## Hardware Requirements
- AMD Radeon 780M (RDNA3, gfx1102) or compatible iGPU
- 16GB+ system RAM (8GB UMA reserved via BIOS)
- AMD driver 26.1.1+ (confirmed: 32.0.31035.1003)
- Windows 11

## Software Requirements
- Python 3.12 (mandatory - AMD only provides cp312 wheels for ROCm 7.2)
- Windows ROCm 7.2 (from repo.radeon.com)
- ACE-Step 1.5 checkpoints downloaded

## Step-by-Step Installation

### 1. Create Isolated ROCm Venv
```bash
...
### 2. Install ROCm SDK Components (~2.5GB, 15+ min)
```cmd
...
### 3. Replace CPU Torch with ROCm PyTorch
```cmd
...
### 5. Configure .env for Radeon 780M (gfx1102)
```ini
...
```cmd
venv_rocm\Scripts\pip install tokenizers==0.22.2 --force-reinstall
```

### Hermes venv conflict
Delete these from Hermes venv site-packages:
- `transformers`
- `tokenizers`
- `huggingface_hub`
- `hf_*`
...
### GPU not detected
- Verify HSA_OVERRIDE_GFX_VERSION=11.0.2 in .env
- Check AMD driver version >= 26.1.1
- Run `venv_rocm\Scripts\python -c "import torch; print(torch.cuda.is_available())"`
