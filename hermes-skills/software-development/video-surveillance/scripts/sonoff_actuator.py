"""Sonoff/Tasmota actuator (MQTT + HTTP control)."""
...
class SonoffActuator(BaseActuator):
    """Sonoff device actuator supporting both Tasmota (MQTT/HTTP) and stock eWeLink LAN mode."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.ip:
            raise ValueError("SonoffActuator requires 'ip' in config")
        ...
        self.mqtt_topic = config.get("mqtt_topic", "cmnd/sonoff")
        self.device_name = config.get("device_name", "sonoff")
    ...
    def _init_mqtt(self):
        ...
        try:
            ...
            print(f"SonoffActuator MQTT connect failed: {e}")
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            print(f"SonoffActuator MQTT connected to {self.mqtt_host}")
        else:
            print(f"SonoffActuator MQTT connect failed: {rc}")
    
    def _on_mqtt_disconnect(self, client, userdata, rc):
        self._mqtt_connected = False
        print(f"SonoffActuator MQTT disconnected: {rc}")
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        try:
            ...
            print(f"SonoffActuator MQTT message error: {e}")
    ...
    def _send_mqtt_command(self, command: str) -> bool:
        ...
        try:
            ...
            print(f"SonoffActuator MQTT publish failed: {e}")
            return False
    ...
    def _send_http_command(self, endpoint: str, payload: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            print(f"SonoffActuator HTTP command failed: {e}")
            return False
    ...
    def _execute_with_fallback(self, mqtt_cmd: str, http_endpoint: str, http_payload: Optional[Dict] = None) -> bool:
        ...
        # Fallback to HTTP
        if self.use_http_fallback and HTTP_AVAILABLE:
    ...
    def turn_on(self) -> bool:
        ...
        try:
            ...
            print(f"SonoffActuator turn_on failed: {e}")
            return False
    ...
    def turn_off(self) -> bool:
        ...
        try:
            ...
            print(f"SonoffActuator turn_off failed: {e}")
            return False
    
    def get_status(self) -> bool:
        """Get current relay status."""
        # If we have cached status from MQTT, use it
        if self._last_status is not None:
            return self._last_status
        
        # Try to query via HTTP
        if HTTP_AVAILABLE and self.use_http_fallback:
            try:
                import requests
    ...
    def get_power(self) -> Optional[float]:
        ...
        try:
            ...
            print(f"SonoffActuator get_power failed: {e}")
        return None
        ...
        try:
            if HTTP_AVAILABLE:
                ...
                if r.status_code == 200:
                    ...
                    if "StatusSNS" in data and "ENERGY" in data["StatusSNS"]:
                        energy = data["StatusSNS"]["ENERGY"]
                        return float(energy.get("Voltage", 0))
        except Exception as e:
            print(f"SonoffActuator get_voltage failed: {e}")
        return None
        ...
        try:
            if HTTP_AVAILABLE:
                ...
                if r.status_code == 200:
                    ...
                    if "StatusSNS" in data and "ENERGY" in data["StatusSNS"]:
                        energy = data["StatusSNS"]["ENERGY"]
                        return float(energy.get("Total", 0))
        except Exception as e:
            print(f"SonoffActuator get_energy failed: {e}")
        return None
...
def create_sonoff_actuator(config: Dict[str, Any]) -> SonoffActuator:
