#!/usr/bin/env python3
"""HERMES VERIFY ALL — Linux version for TurboCoder / Monster Coder"""
...
GRIMOIRE_SCRIPTS = Path.home() / "GodModeCoder" / "scripts"
GRIMOIRE_CONFIG = Path.home() / "GodModeCoder" / "configs" / "graph.yaml"
...
def test_vault_tags():
    ...
    for md_file in node_files:
        try:
            result = Frontmatter.read_file(str(md_file))
            tags = set(result.get('attributes', {}).get('tags', []))
            if not required_tags.issubset(tags):
                print(f"{Colors.YELLOW}[TEST] Tags WARN{Colors.RESET} ({md_file.name}: missing #evolution/graph, has {tags})")
                return True  # Warning, not fail
            if not any(t in tags for t in type_tags):
                print(f"{Colors.YELLOW}[TEST] Tags WARN{Colors.RESET} ({md_file.name}: missing #type/*, has {tags})")
                return True
            if not any(t in tags for t in status_tags):
                print(f"{Colors.YELLOW}[TEST] Tags WARN{Colors.RESET} ({md_file.name}: missing #status/*, has {tags})")
                return True
        except Exception as e:
            print(f"{Colors.YELLOW}[TEST] Tags WARN{Colors.RESET} ({md_file.name}: {e})")
            return True  # Warning for encoding issues
...
def main():
    ...
    print(f"  HERMES VERIFY ALL — TurboCoder / Monster Coder Auto-Test Suite (Linux)")
