...
## 🌆 TOP BANNER — CYBERPUNK VAN GOGH GAUDI STYLE
...
```bash
# 1. Verify your Cathedral (run in NEW terminal after setup)
bash cathedral-verify.sh

# 2. Initialize a project in 30 seconds
...
# 3. Dev loop
cd my-app
./cathedral-check.sh          # All quality gates: fmt + lint + test + security
flutter run -d chrome         # Hot reload web (or your stack equivalent)
...
|-----------|-------------|
| **CATHEDRAL_CODE_MACHINE.md** | Full specification (379 lines) — architecture, lifecycle, gates, evolution |
| **cathedral-verify.sh** | System verification — 26 checks across all toolchains |
| **cathedral-init.sh** | Project generator — 6 stacks, CI/CD, quality gates, observability |
| **Cathedral-QuickRef.ps1** | PowerShell reference card for daily use |
| **Setup scripts** | `add-*.ps1` — permanent registry PATH/env vars for all tools |

---

## 🛠️ SUPPORTED STACKS (ALL IN PATH, PERMANENT)
...
```
BRIEF → SPEC → ARCHITECTURE → SKELETON → CI/CD
  │
  ...
  │
  ├─► DEBUGGING (Best tool per language)
  │     ├─► Rust: rust-gdb/lldb | Kotlin: IntelliJ | Flutter: DevTools
  │     └─► C++: VS Debugger/LLDB | Android: AS Profiler | System: WPA
  │
  ├─► RELEASE AUTOMATION
  │     └─► git tag v1.0.0 → GitHub Actions → All Artifacts → Stores
  │
  └─► EVOLUTION (Auto, every 6h via cron)
        ├─► Security audit (cargo audit, govulncheck, trivy)
        ├─► Performance regression detection
        ├─► API compatibility (protobuf)
        ├─► Architecture drift detection
        └─► Reports → C:\Vault\Evolution\ (Git history = evolution log)
```
...
```
Cathedral/
├── CATHEDRAL_CODE_MACHINE.md      # Full specification
├── cathedral-verify.sh            # System verification
├── cathedral-init.sh              # Project generator
├── Cathedral-QuickRef.ps1         # PowerShell reference
├── add-*.ps1                      # Registry setup scripts
├── README.md                      # English (this)
...
## 🎨 BOTTOM BANNER — CYBERPUNK VAN GOGH GAUDI STYLE
...
MIT License — Use freely, build cathedrals.
